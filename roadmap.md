...
## PHASE 1 — Project Setup & App Shell (≈ 1 week)
...
* Define Room database class (empty schema, migration strategy)

### 1.3 Frontend Tasks — Screens Built

* **Splash / Welcome Screen** — app icon, tagline, auto‑navigate to onboarding or home
* **App Shell** — `Scaffold` with `TopAppBar`, `NavigationBar` (Home, Transfers, Settings)
* **Theme Setup** — Material 3 dynamic color, light/dark mode, typography scale
* Navigation graph with all route stubs (empty placeholder screens)
...
## PHASE 2 — Telegram Bot + Channel Setup (≈ 2–3 weeks)
...
* Implement `TelegramApiService` (OkHttp): `getMe`, `sendDocument`, `getFile`
* Validate bot token via `getMe` call
* Upload test file to channel via `sendDocument`
* Download file via `getFile` → `file_path` URL
* Store bot token encrypted (Android Keystore) in Room `metadata` table
...
* **Onboarding Screen (3 steps)**
  * Step 1: "How it works" — illustration + brief explanation
  * Step 2: "Enter Bot Token" — `OutlinedTextField`, paste support, "How to create a bot?" link
  * Step 3: "Verify Channel" — enter channel ID or username, verify bot is admin, success animation
* **Onboarding states**: loading spinner during verification, error `Snackbar` with retry, success transition to Home
* Bot token input uses `visualTransformation` to mask token after entry

### 2.4 Success Criteria

* User completes onboarding and reaches Home
* App uploads to channel and downloads by `file_id`
* Configuration survives app restart
* Onboarding shows clear errors for invalid token or channel

---

## PHASE 3 — Local Database & File Browser (≈ 2 weeks)
...
* Repository: `FileRepository` (insert, query, search, delete)
* Local file import: copy picked file to app‑private storage, compute SHA‑256
* Thumbnail generation for images (BitmapFactory downscale)
...
* **Home — File Browser**
  ...
  * `FilterChip` row — All, Images, Documents, Videos, Audio, Other
  * Each file item: `Card` with thumbnail, name, size, sync badge icon
  * `FloatingActionButton` → navigate to Upload
  * **Empty state**: friendly illustration + "Upload your first file" CTA
  * **Offline indicator**: `TopAppBar` subtitle or banner when no network
  * Pull‑to‑refresh gesture
* **File Detail / Preview Screen**
  * File name, size, type, created date
  * Sync status chip (uploaded / pending / failed)
  * Thumbnail or icon preview
  * Actions: Download, Delete, Re‑upload, Share (local)
